;https://www.reddit.com/r/AutoHotkey/comments/jepw7s/asetniop/
...
SENDX:
   ...
   SentKeys = %key0%
	...
	Else IfEqual key0,AK, Send Z{Shift Up}
	...
	Else IfEqual key0,SK, Send K{Shift Up}
	...
	Else IfEqual key0,K:, Send !{Shift Up}
	...
	Else IfEqual key0,QI, Send Z{Shift Up}
	...
	Else IfEqual key0,WI, Send K{Shift Up}
	...
	Else IfEqual key0,IP, Send !{Shift Up}
...
;	Else IfEqual key0,, Send
	Else IfEqual key0,dfjk, Send the
	Else IfEqual key0,dfj, Send be
	Else IfEqual key0,afl, Send of
	Else IfEqual key0,asdj, Send and
	Else IfEqual key0,afjk, Send that
	Else IfEqual key0,adfjk, Send have
	Else IfEqual key0,adfl, Send for
	Else IfEqual key0,fjl, Send not
	Else IfEqual key0,djl, Send you
	Else IfEqual key0,sdl, Send do
	Else IfEqual key0,sfjk, Send this
	Else IfEqual key0,sjk, Send his
	Else IfEqual key0,adfjl;, Send from
	Else IfEqual key0,asd, Send we
	Else IfEqual key0,sdjk, Send she
	Else IfEqual key0,dfl, Send or
	Else IfEqual key0,askl, Send will
	Else IfEqual key0,dj;, Send my
	Else IfEqual key0,akl, Send all
	Else IfEqual key0,asdjkl, Send would
	Else IfEqual key0,dfjkl, Send their
	Else IfEqual key0,asfjk, Send with
	Else IfEqual key0,jl;, Send up
	Else IfEqual key0,afk, Send if
	Else IfEqual key0,afjl, Send about
	Else IfEqual key0,asjkl, Send who
	Else IfEqual key0,asfjkl, Send which
	Else IfEqual key0,asdjk, Send when
	Else IfEqual key0,asdjk;, Send make
	Else IfEqual key0,asfj, Send can
	Else IfEqual key0,djkl;, Send like
	Else IfEqual key0,dfjk;, Send time
	Else IfEqual key0,sfjl, Send just
	Else IfEqual key0,jk;, Send him
	Else IfEqual key0,asdfk, Send take
	Else IfEqual key0,afjk, Send than
	Else IfEqual key0,asjl, Send now
	Else IfEqual key0,skl, Send look
	Else IfEqual key0,djkl, Send only
	Else IfEqual key0,sdfjl;, Send come
	Else IfEqual key0,sfk, Send its
	Else IfEqual key0,dfkl, Send over
	Else IfEqual key0,sfjkl, Send think
	Else IfEqual key0,adf, Send after
	Else IfEqual key0,sdjl, Send use
	Else IfEqual key0,asfl, Send two
	Else IfEqual key0,asjkl, Send how
	Else IfEqual key0,dfjl, Send our
	Else IfEqual key0,asdfkl, Send work
	Else IfEqual key0,asdfk, Send first
	Else IfEqual key0,asdkl, Send well
	Else IfEqual key0,asdj, Send way
	Else IfEqual key0,asfj, Send want
	Else IfEqual key0,sfj, Send because
	Else IfEqual key0,adj, Send any
	Else IfEqual key0,sdfjk, Send these
	Else IfEqual key0,dfjkl, Send give
	Else IfEqual key0,asdj, Send day
	Else IfEqual key0,sfjl;, Send most
	Else IfEqual key0,sjl, Send us
	Else IfEqual key0,djk, Send he
	Else IfEqual key0,aj;, Send am
	Else IfEqual key0,fjkl, Send into
	Else IfEqual key0,asdfjk, Send death
	Else IfEqual key0,sdfjkl;, Send something
	Else IfEqual key0,sdfjkl, Send others
	Else IfEqual key0,asdk, Send said
	Else IfEqual key0,asdfl, Send word
	Else IfEqual key0,adfk, Send air

	Else IfEqual key0,asdf, Send {Tab}
	Else IfEqual key0,jkl;, Send {Enter}
